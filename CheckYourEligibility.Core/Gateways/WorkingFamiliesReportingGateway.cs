...
public class WorkingFamiliesReportingGateway : IWorkingFamiliesReporting
{
    ...
    public async Task<WorkingFamilyEventByEligibilityCodeResponse> GetAllWorkingFamiliesEventsByEligibilityCode(string eligibilityCode)
    {
        try
        {
            ...
            var result = new List<WorkingFamilyEventByEligibilityCodeResponseItem>();
            ...
            foreach (var current in records)
            {
                ...
                result.Add(new WorkingFamilyEventByEligibilityCodeResponseItem
                {
                    Event = isReconfirm
                        ? WorkingFamilyEventType.Reconfirm
                        : WorkingFamilyEventType.Application,

                    Record = _mapper.Map<WorkingFamiliesEventEligibilityCodeResponseRecord>(current)
                });
