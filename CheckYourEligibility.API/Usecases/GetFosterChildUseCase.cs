...
{
    Task<FosterChildResponse> Execute(Guid fosterChildId, bool includeFosterCarer = false);
}
...
public class GetFosterChildUseCase : IGetFosterChildUseCase
{
    ...
    public async Task<FosterChildResponse> Execute(Guid fosterChildId, bool includeFosterCarer = false)
    {
        if (fosterChildId == Guid.Empty) throw new ValidationException("A valid fosterChildId is required");

        var result = await _gateway.GetFosterChild(fosterChildId, includeFosterCarer);
